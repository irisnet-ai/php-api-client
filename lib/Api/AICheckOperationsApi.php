...
/**
     ...
     * @param  string $url The url to the image that needs to be checked. (required)
     ...
     */
    public function checkImage($configId, $url, $detail = 1, $imageEncode = false, string $contentType = self::contentTypes['checkImage'][0])
    {
        list($response) = $this->checkImageWithHttpInfo($configId, $url, $detail, $imageEncode, $contentType);
        return $response;
    }
     ...
     */
    public function checkImageWithHttpInfo($configId, $url, $detail = 1, $imageEncode = false, string $contentType = self::contentTypes['checkImage'][0])
    {
        $request = $this->checkImageRequest($configId, $url, $detail, $imageEncode, $contentType);
     ...
     */
    public function checkImageAsync($configId, $url, $detail = 1, $imageEncode = false, string $contentType = self::contentTypes['checkImage'][0])
    {
        return $this->checkImageAsyncWithHttpInfo($configId, $url, $detail, $imageEncode, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function checkImageAsyncWithHttpInfo($configId, $url, $detail = 1, $imageEncode = false, string $contentType = self::contentTypes['checkImage'][0])
    {
        $returnType = '\Irisnet\API\Client\Model\CheckResult';
        $request = $this->checkImageRequest($configId, $url, $detail, $imageEncode, $contentType);
     ...
     */
    public function checkImageRequest($configId, $url, $detail = 1, $imageEncode = false, string $contentType = self::contentTypes['checkImage'][0])
    {
        ...
        if ($url === null || (is_array($url) && count($url) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $url when calling checkImage'
            );
        }
